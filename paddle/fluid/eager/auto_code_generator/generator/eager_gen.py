...
{indent}{indent}if (paddle::experimental::GetDebugStartStr() != "") {{
{indent}{indent}{indent}std::cout << paddle::experimental::GetDebugStartStr() << "in: " << debug_str << std::endl;
{indent}{indent}}}
{indent}}}
...
"""
        ...
        api_output_type_map = {
            "Tensor": "paddle::experimental::Tensor*",
            "std::vector<Tensor>": "std::vector<paddle::experimental::Tensor*>",
        }
        transformed_tensor_name_list = []
        for name, (
            ttype,
            fwd_position,
            grad_api_position,
  ...
  for (size_t i = 0; i < returns[{fwd_position}].size(); ++i) {{
    if (out_metas[{fwd_position}].empty() || out_metas[{fwd_position}][i].IsStopGradient()) {{
      ...
      api_output_{out_index}.push_back(&returns[{fwd_position}][i]);
            ...
            # transformed_tensor_name_list.append(transformed_tensor_name)
            ...
            for input_name, type in input_name_type_map.items():
                if type == "Tensor":
                    dev2_input_list.append(
                        f'paddle::experimental::Tensor dev2_{input_name};'
                    )
                    ...
                    dev2_input_list.append(
                        f'std::vector<paddle::experimental::Tensor> dev2_{input_name};'
                    )
                    dev2_input_copy_list.append(
                        f'paddle::experimental::copy({input_name}, paddle::experimental::xpu_debug_run_dev2(), false, &dev2_{input_name});'
                    )
                    ...
                    if grad_api_args[i] in api_output_name_type_dict.keys():
                        ...
                        if (
                            ...
                            op_debug_grad_tensor_copy_list.append(
                                f"""
{indent}paddle::experimental::Tensor dev2_{grad_api_args[i]}_tensor;
...
{indent}std::vector<std::shared_ptr<paddle::experimental::Tensor>> dev2_{grad_api_args[i]}_tmp;
...
{indent}    // paddle::experimental::Tensor dev2_{grad_api_args[i]}_tensor = new paddle::experimental::Tensor();
{indent}  auto dev2_{grad_api_args[i]}_tensor = std::make_shared<paddle::experimental::Tensor>();
